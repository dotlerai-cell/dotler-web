...
export const generateCaption = async (topic: string, base64Image?: string): Promise<{ caption: string; hashtags: string[] }> => {
    try {
...
export const suggestRelatedHashtags = async (topic: string, currentHashtags: string[]): Promise<string[]> => {
    try {
 ...
 */
export const retrieveRelevantDocuments = (query: string, docs: KnowledgeDocument[], topK: number = 3): string[] => {
...
export const generateDMBotResponse = async (
